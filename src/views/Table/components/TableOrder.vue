            ...
            <n-scrollbar>
                <n-grid responsive="screen" cols="1 s:1 m:5 l:5 xl:5 2xl:5" :x-gap="12"
                    ...
                    <n-gi span="2">
                        ...
                        : 1
                        " label="Cliente">
                                        <n-input v-model:value="ask_for" placeholder=""
                                            :readonly="userStore.user.role === 'MOZO'"
                                            :disabled="userStore.user.role === 'MOZO'" />
                                    </n-form-item-gi>
                                    <n-form-item-gi
                                        v-if="settingsStore.businessSettings?.order?.select_order_user && userStore.user.role !== 'MOZO'"
                                        :span="!settingsStore.businessSettings?.order?.order_customer_name || shouldShowCustomerMode
                                            ? 2
                                            : 1
                                            " label="Mozo">
                                        <n-select :options="activeUsersStore.usersOptions" v-model:value="orderUser"
                                ...
                                <div v-if="shouldShowCustomerForm">
                                    <n-form-item label="Agregar Cliente">
                                        <n-input-group>
                                            ...
                                            <n-button type="primary" @click="addCustomer"
                                ...
                                <n-form-item label="Buscar producto" v-if="shouldAllowAddingProducts">
                                    <n-input-group>
                                        <n-auto-complete :input-props="{
                                            autocomplete: 'disabled',
                                        }" v-model:value="productSearch" :options="productOptions" :get-show="showOptions"
                                            :loading="searching" placeholder="" clear-after-select
                                            :render-label="renderLabel" @select="selectProduct" />
                                    </n-input-group>
                                </n-form-item>
                            </n-form>
                                        ...
                                        ">
                                            ...
                                            <td>
                                                <n-input-number v-if="!($route.name === 'TablePayment')"
                                                    class="border-top-0" size="small" :min="order.id ? saleStore.getOrderQuantity(order.id) : 1
                                    ...
                                    <tr>
                                        <td colspan="3">
                                            <n-button v-if="!($route.name === 'TablePayment')"
                                                :type="orderStore.orderId ? 'info' : 'primary'" text block
                                                :loading="loading" :disabled="orderStore.orderList.length
                                                    ? checkState || loading
                                                    : true
                                                    " @click="validateSend()">
                            ...
                            <div v-else>
                                    ...
                                    <n-card class="mb-3" size="small" :title="customer.name" :bordered="true"
                                        ...
                                        <template #header-extra>
                                            <n-space>
                                                ...
                                                <n-button v-if="!($route.name === 'TablePayment')" type="error"
                                                    size="small"
                                ...
                                <n-empty v-if="customers.length === 0" description="No hay clientes agregados">
                                    <template #extra>
                                        <n-text depth="3">
                                            Agregue un cliente usando el campo "Agregar Cliente" para comenzar a tomar
                                            pedidos.
                                        </n-text>
                                    </template>
                                </n-empty>
...
export default defineComponent({
    ...
    setup() {
        ...
        const performCreateTableOrder = async () => {
            loading.value = true;
            console.log(JSON.stringify(orderStore.orderList));
