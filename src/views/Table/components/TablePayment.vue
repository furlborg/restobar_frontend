            ...
            <n-form-item-gi :span="9" label="Cliente" :show-require-mark="formRules.customer.required" path="customer">
              <n-input-group>
                <n-auto-complete blur-after-select :input-props="{
                  ...
                  :loading="searching" @keypress.enter="autoCreateCustomer" @update:value="
                    (v) => {
                      !v
                        ? ((sale.customer = null),
                          (sale.address = null),
                ...
                <tr v-if="detail.quantity > 0" :key="index">
                  ...
                  <td>
                    ...
                    <input class="custom-input" type="number" min="0" step=".5" v-model="detail.price_sale"
                      @input="(v) => (saleStore.updateDetail(detail), detail.discount = parseFloat(0).toFixed(2))"
            ...
            <n-space class="mt-2 fs-6 fw-bold" align="end" vertical>
              ...
              <div>
                ...
                <input class="custom-input fw-bold" type="number" min="0" step=".5" v-model="totalDSCT" v-autowidth
                  :disabled="saleStore.toSale.some(detail => Number(detail.discount) > 0)"
                  @click="$event.target.select()" />
              </div>
              <div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const total = computed(() => {
      return parseFloat(
        totalIGV.value + totalGRV.value + totalEXN.value - totalGRT.value -
