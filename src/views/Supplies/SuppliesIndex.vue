<template>
  <div>
    <n-card title="Insumos" :segmented="{ content: 'hard' }">
      ...
      <n-data-table :columns="tableColumns" :data="supplies" size="small" :scroll-x="900" :loading="isLoadingData"
        remote :pagination="pagination" />
    </n-card>
    <!-- Customer Modal -->
    <SuppliesModal v-model:show="showModal" @on-success="listSupplies()" :items="items" />
    <MoveModal v-model:show="showModalMovement" @on-success="listSupplies()" :items="itemsMovement" :type="type" />
  </div>
</template>
