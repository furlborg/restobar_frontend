<template>
  <div id="Combo">
    <!--Card Principal de combos -->
    <n-card title="Combos" :segmented="{ content: 'hard' }">
      <template #header-extra>
        <n-space>
          <n-button v-if="canAddCombo" type="success" @click="goCreate" aria-label="Crear nuevo combo">
            <template #icon>
      ...
      <n-space justify="space-between" class="mb-3">
        ...
        <div class="d-flex">
          <n-button type="info" text @click="fetchCombos" aria-label="Recargar lista">
        ...
        <n-form>
          <n-grid responsive="screen" cols="6 s:6 m:12 l:12 xl:24 2xl:24" :x-gap="12">
            ...
            <n-form-item-gi label="Categoría" :span="6">
              <n-select v-model:value="filterParams.category_id" :options="categoryOptions"
                placeholder="Todas las categorías" clearable filterable />
            </n-form-item-gi>
            <n-form-item-gi label="Modo de precio" :span="5">
              <n-select v-model:value="filterParams.pricing_mode" :options="PRICING_MODE_OPTIONS" placeholder="Todos"
                clearable />
            </n-form-item-gi>
            <n-form-item-gi :span="5">
              <n-button type="info" @click="applyFilters">Buscar</n-button>
            </n-form-item-gi>
          </n-grid>
        </n-form>
      </n-collapse-transition>

      <n-data-table class="mt-3" :columns="tableColumns" :data="combos" :loading="isTableLoading"
        :pagination="pagination" :row-key="row => row.id" remote @update:page="handlePageChange"
        @update:page-size="handlePageSizeChange" />
    </n-card>

    <!-- Modal Create/Edit -->
    <ComboFormModal v-model:show="showFormModal" :combo-id="selectedComboId" :mode="formMode"
...
/**
 * Construye los parámetros para la petición, eliminando valores vacíos
 */
const buildApiParams = () => {
  ...
  // Filtra valores nulos, undefined o cadena vacía
  return Object.fromEntries(
    Object.entries(baseParams).filter(entry =>
...
const fetchCombos = async () => {
  ...
  try {
    ...
    // Ajusta según la estructura real de tu API (paginada o no)
    const result = response.data
    combos.value = result.data || result  // Si es paginado, result.data contiene el array
    pagination.itemCount = result.total || result.count || combos.value.length
...
// ---------- Definición de columnas de la tabla (computado) ----------
const tableColumns = computed(() => {
  ...
  // Agregar columna de acciones solo si hay permisos
...
// ---------- Carga inicial ----------
onMounted(async () => {
  await Promise.all([fetchCombos(), fetchCategories()])
})
</script>
