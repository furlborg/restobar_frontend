...
export class printTicketSalesAnulate {
  ...
  addInformation(dataPrint) {
    dataPrint?.forEach((v) => {
      ...
      let textDocument = this.doc.splitTextToSize(
        `${v?.["invoice_type_description"]} ${v?.["serie"]}-${v?.["number"]}`,
        50,
      );
      ...
      this.doc.text(":", dots_x, y);
      this.doc.setFont(undefined, "normal");
      let textObservation = this.doc.splitTextToSize(
        v.null_reason || "SIN OBSERVACIONES",
        50,
      );
