...
export class printTicketOrderAnulate {
  ...
  async add_header(doc, type) {
    ...
    this.doc.text(
      `${business.value.commercial_name || "xd"}`.toUpperCase(),
      x + 10,
      y + 2,
    );
    y += 6;
    ...
    this.doc.setFontSize(10);
    let addressSplitText = this.doc.splitTextToSize(
      business.value.fiscal_address || "ekizde",
      75,
    );
    ...
    this.doc.text(
      `REPORTE ${type === "T" ? "TOTAL" : "PARCIAL"} DE ORDENES \n ANULADAS`,
      x + 38,
      y + 2,
      "center",
    );
  ...
  addInformation(dataPrint, type) {
    dataPrint?.forEach((v) => {
      ...
      this.doc.setFont(undefined, "normal");
      this.doc.text(
        v?.["table_description"] || "yape dany sube el cambio de las mesas",
        data_x,
        y,
      );
      y += 5;
      ...
      const totalOrdersCalculate = v?.["order_canceleddetails"].reduce(
        (a, b) => a + parseFloat(b?.["sub_total"]),
        0,
      );
      ...
      this.doc.setFont(undefined, "normal");
      this.doc.text(
        "S/. " + parseFloat(totalOrdersCalculate)?.toFixed(2) || "0.00",
        data_x,
        y,
      );
      y += 5;
