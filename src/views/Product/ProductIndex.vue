<template>
  <div id="Product">
    <n-card title="Productos" :segmented="{ content: 'hard' }">
      <template #header-extra>
        <n-space justify="space-around">
          ...
          <n-button v-if="userStore.hasPermission('add_stock')" type="success"
            @click="newMovement(0), (showModalMovement = true)" secondary>
            <template #icon><n-icon><v-icon name="hi-solid-arrow-sm-up" /></n-icon></template>
            Entrada
          </n-button>
          <n-button v-if="userStore.hasPermission('remove_stock')" type="error" secondary
            @click="newMovement(1), (showModalMovement = true)">
            <template #icon><n-icon><v-icon name="hi-solid-arrow-sm-down" /></n-icon></template>
            Salida
          </n-button>
          <n-button v-if="userStore.hasPermission('add_product')" type="primary" @click="showModal = true" secondary>
            <template #icon><n-icon><v-icon name="la-user-plus-solid" /></n-icon></template>
            Crear
          </n-button>
        </n-space>
      </template>
      ...
      <n-spin class="mt-2" :show="isLoadingData">
        <n-scrollbar style="height: 70vh; min-height: 650px">
          ...
          <n-list v-if="listType === 'list'" class="px-3">
            <n-list-item v-for="product in products" :key="product.id" @mouseover="product.showButtons = true"
              @mouseleave="product.showButtons = false">
              <template #prefix>
                <img :src="productImage(product)" alt width="90" height="90" />
              </template>
              <n-thing>
                <template #header>
                  <n-text class="fs-4" :type="product.is_disabled ? 'error' : 'default'">{{ product.name }}</n-text>
                </template>
              ...
              <template #suffix>
                <transition name="fade">
                  <n-button-group v-if="product.showButtons">
                    <n-button v-if="userStore.hasPermission('change_product')" class="p-4" type="warning" tertiary
                      @click="editProduct(product.id)">
                      <v-icon name="ri-edit-fill" scale="1.5" />
                    </n-button>
                    <n-button v-if="userStore.hasPermission('delete_product')" class="p-4"
                      :type="product.is_disabled ? 'success' : 'error'" tertiary @click="
                        performDisableProduct(product.id, product.is_disabled)
                        ">
                      <v-icon :name="product.is_disabled
          ...
          <!-- Products Grid -->
          <n-grid v-if="listType === 'grid'" class="mt-2" responsive="screen" cols="4 s:4 m:12 l:12 xl:24 2xl:24"
            :x-gap="12" :y-gap="12">
            <n-gi :span="4" v-for="product in products" :key="product.id">
              <n-card>
                <template #header>
                  <n-h2 class="mb-0">{{ product.name }}</n-h2>
                  <n-text type="success">S/. {{ Number(product.prices).toFixed(2) }}</n-text>
                </template>
                <template #header-extra>
      ...
      <template #footer>
        <n-space justify="end">
          <!-- Products Paginator -->
          <n-pagination :page="pagination.page" :page-count="pagination.pageCount" :page-size="pagination.pageSize"
            :page-sizes="pagination.pageSizes" :on-update:page="pagination.onChange"
            :on-update:page-size="pagination.onPageSizeChange" show-size-picker />
        </n-space>
      </template>
    </n-card>
    ...
    <move-modal v-model:show="showModalMovement" @on-success="loadProductsData" :items="itemsMovement" :type="type" />
  </div>
</template>
