<template>
  <div>
    <n-card title="Proveedores" :segmented="{ content: 'hard' }">
      ...
      <n-data-table :columns="tableColumns" :data="supplier.results" size="small" :scroll-x="900"
        :loading="isLoadingData" remote :pagination="pagination" />
    </n-card>
    <!-- Customer Modal -->
    <supplier-modal v-model:show="showModal" @on-success="listSupplier" :items="items" />
  </div>
</template>
...
const tableColumns = createSuppliersColumns({
  editSupplier(data) {
    editSupplier(data);
