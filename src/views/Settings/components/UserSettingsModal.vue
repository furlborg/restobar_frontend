    ...
    >
                    ...
                    <n-tab-pane name="info" tab="Información General">
                        <n-grid cols="12 100:1 450:12" :x-gap="12">
                            <n-form-item-gi label="Nombres" :span="4" path="names">
                                <n-input v-model:value="formitem.names" placeholder="Nombres" :maxlength="200" @input="formitem.names = $event.toUpperCase()" />
                            </n-form-item-gi>
                            <n-form-item-gi label="Sucursal" :span="4" path="branchoffice">
                                <n-select v-model:value="formitem.branchoffice" placeholder="Seleccione" :options="optionsBranch" />
                            </n-form-item-gi>
                            <n-form-item-gi label="Perfil" :span="4" path="profile">
                                <n-select v-model:value="formitem.profile" placeholder="Seleccione" :options="optionsProfile" />
                            </n-form-item-gi>
                        </n-grid>
                        <n-grid cols="12 100:1 450:12" :x-gap="12">
                            <n-form-item-gi label="DNI" :span="4" path="dni">
                                <n-input v-model:value="formitem.dni" placeholder="DNI" :maxlength="8" />
                            </n-form-item-gi>
                            <n-form-item-gi label="Corrreo Electrónico" :span="8">
                                <n-input v-model:value="formitem.email" placeholder="Correo@ejemplo.com" :maxlength="200" />
                            </n-form-item-gi>
                        </n-grid>
                        <n-grid cols="12 100:1 450:12" :x-gap="12">
                            <n-form-item-gi label="Usuario" :span="4" path="username">
                                <n-input v-model:value="formitem.username" placeholder="Usuario" :maxlength="11" @input="formitem.username = $event.toUpperCase()" />
                            </n-form-item-gi>
                            ...
                            <n-form-item-gi v-if="!items.id" label="Confirmar Contraseña" :span="4">
                                <n-input type="password" v-model:value="confirmPass" :maxlength="10" show-password-on="click" placeholder="Confirmar contraseña" />
                            </n-form-item-gi>
                        </n-grid>
                    </n-tab-pane>
                    <n-tab-pane name="permissions" tab="Permisos">
                        <n-tree cascade checkable :data="permissions" :default-checked-keys="formitem.user_permission" @update:checked-keys="updateCheckedKeys" />
                    </n-tab-pane>
                </n-tabs>
            </n-form>
        ...
        <template #action>
            <n-space justify="end">
                ...
                <n-button type="primary" @click="save(formitem)">{{ items.id ? "Guardar" : "Crear" }}</n-button>
            </n-space>
        </template>
    </n-modal>
</template>
...
import { getProfile, createUsers, updateUsers, getPermission } from "@/api/modules/users";
    ...
    setup(props, { emit }) {
        ...
        onUpdated (() => {
            if (show.value == true) {
                ...
                if (formitem.value.user_permission) {
                    formitem.value.user_permission = formitem.value.user_permission.map((v) => {return v.permission})
                }
            }
        })
        ...
        const save = (formitem) => {
            formRef.value.validate(async (errors) => {
                if (!formitem.names || !formitem.username || !formitem.dni || !formitem.branchoffice || !formitem.profile) {
                    message.warning("Campos Requeridos");
                }else if(formitem.user_permission == undefined || formitem.user_permission.length == 0){
