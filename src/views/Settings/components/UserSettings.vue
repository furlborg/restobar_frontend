  ...
  <div id="UserSettings">
    ...
    <n-card title="Usuarios" :segmented="{ content: 'hard' }">
      <template #header-extra>
        <n-space>
          <n-button v-if="userStore.user.is_owner" type="warning" secondary @click="showPassModal = true">Cambiar clave
            de seguridad</n-button>
          <n-button v-if="userStore.hasPermission('add_user')" type="primary" @click="newUser(), (showModal = true)"
            secondary>
            <template #icon>
              <n-icon>
      ...
      <n-data-table class="mt-2" :columns="tableColumns" :data="users.results" size="small" :scroll-x="900"
        :loading="isLoadingData" remote :pagination="pagination" />
    </n-card>
    <!-- Customer Modal -->
    <user-settings-modal v-model:show="showModal" @on-success="listUsers" :items="items" />
    <user-password v-model:show="showModalPass" @on-success="listUsers" :items="itemsUser" />
    <n-modal :class="{
      'w-100': genericsStore.device === 'mobile',
      'w-50': genericsStore.device === 'tablet',
      'w-25': genericsStore.device === 'desktop',
    }" preset="card" v-model:show="showPassModal" title="Cambiar clave de seguridad" :mask-closable="false" closable
      ...
      <n-form ref="securityForm" :model="securityPass" :rules="securityRules">
        <n-form-item label="Ingrese clave actual" path="currentPass">
          <n-input type="password" v-model:value="securityPass.currentPass" placeholder="" />
        </n-form-item>
        <n-form-item label="Ingrese nueva clave" path="newPass">
          <n-input type="password" v-model:value="securityPass.newPass" @input="handlePasswordInput" placeholder="" />
        </n-form-item>
        <n-form-item label="Reingrese nueva clave" ref="rePassRef" path="rePass">
          <n-input type="password" v-model:value="securityPass.rePass" placeholder="" />
        </n-form-item>
      </n-form>
      <template #action>
        <n-space justify="end">
          <n-button type="success" :loading="isLoading" :disabled="!securityPass.currentPass ||
            !securityPass.newPass ||
            !securityPass.rePass ||
            isLoading
            " secondary @click.prevent="performUpdateSecurityPass">Confirmar</n-button>
        </n-space>
      </template>
    </n-modal>
