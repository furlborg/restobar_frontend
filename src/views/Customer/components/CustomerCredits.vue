<template>
  <n-drawer-content title="Créditos" body-content-style="padding: 5px;" :native-scrollbar="false" closable>
    <n-space class="mb-1" align="center" justify="space-between">
      <n-input-group v-if="tillStore.currentTillID && filters.status === 'P'">
        <n-select style="max-width: 200px" :disabled="loading" v-model:value="payment.payment_method"
          :options="saleStore.getPaymentMethodsOptions" />
        <n-input-number style="max-width: 200px" :disabled="loading || filters.status !== 'P'"
          v-model:value="payment.amount" placeholder="0.00" :show-button="false" step="0.1" min="0" readonly
          @update:value="amountChanged" />
        <n-button type="success" secondary :loading="loading"
          :disabled="loading || !payment.amount || filters.status !== 'P'" @click="generatePayment">
          Pagar
        </n-button>
      </n-input-group>
      <n-input v-if="filters.status === 'C'" class="me-1" v-model:value="filters.document" placeholder="Documento"
        clearable @update:value="
          (v) => {
            payment = { payment_method: 1, amount: null };
            debouncedLoadCredits(v);
          }
        " />
      <n-select class="me-1" v-model:value="filters.status" placeholder="Estado" :options="statusOptions" @update:value="
        (v) => {
          payment = { payment_method: 1, amount: null };
          debouncedLoadCredits(v);
        }
    ...
    <n-data-table :checked-row-keys="selectedCredits" :loading="loading" :data="credits" :columns="columns"
      :row-key="(row) => row.id" @update:checked-row-keys="generateAmountFromChecked" />
  </n-drawer-content>
</template>
