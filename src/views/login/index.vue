    ...
    formRef.value.validate(async(errors) => {
        if(!errors) {
            ...
            await login(username, password).then(async(response) => {
                ...
                loading.value = false;
                const routeReturn = genericsStore.device === "desktop" ? "TableHome" : "WaiterMode";
                switch(userStore.user.role) {
                    case "MOZO":
                        console.log(await router.push({ name: routeReturn, force: true }));
                        await router.push({ name: routeReturn, force: true });
                        break;
                    case "COCINERO":
                        await router.push({ name: "ChefMode" });
                        break;
                    default:
