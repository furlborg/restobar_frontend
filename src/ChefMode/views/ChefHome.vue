<template>
  <div class="h-100 mh-100" id="CHome">
    <teleport to="body">
      <n-button class="position-absolute end-0 right-0 rounded-0" style="height: 64px; z-index: 20" type="info"
        secondary>
        <v-icon name="hi-solid-refresh" scale="1.5" :animation="refreshing ? 'spin' : null" />
      </n-button>
    </teleport>
    <n-grid class="h-100 mh-100" responsive="screen" cols="3 xs:1 s:3">
      <n-gi>
        <n-card class="h-100 mh-100 pendient" title="Pendientes" :bordered="false">
          <div class="h-100 mh-100 position-relative">
            <n-scrollbar class="position-absolute h-100 mh-100">
              <div class="h-100 mh-100">
                <TransitionGroup name="list">
                  <div class="mb-2" style="background-color: white" v-for="order in pendients" :key="order.id">
                    <div class="p-3">
                      ...
                      <ul>
                        <li v-for="(indication, index) in order.indication" :key="index">
                          <n-text>{{ indication.description }}</n-text>
                        </li>
                      </ul>
                      <n-text v-if="order.ask_for" type="info">CLIENTE: {{ order.ask_for }}</n-text>
                    </div>
                    <n-button class="rounded-0" type="info" secondary block
      ...
      <n-gi>
        <n-card class="h-100 mh-100 preparing" title="Preparando" :bordered="false">
          <div class="h-100 mh-100 position-relative">
            <n-scrollbar class="position-absolute h-100 mh-100">
              <div class="h-100 mh-100">
                <TransitionGroup name="list">
                  <div class="mb-2" style="background-color: white" v-for="order in preparing" :key="order.id">
                    <div class="p-3">
                      <n-space justify="space-between">
                        <n-text type="info">{{ order.user }}</n-text>
                        <n-text type="info">{{ order.time }}</n-text>
                    ...
                    <n-button class="w-100 rounded-0" type="success" secondary
      ...
      <n-gi>
        <n-card class="h-100 mh-100 finished" title="Despachados" :bordered="false">
          <div class="h-100 mh-100 position-relative">
            <n-scrollbar class="position-absolute h-100 mh-100">
              <div class="h-100 mh-100">
                <TransitionGroup name="list">
                  <div class="mb-2" style="background-color: white" v-for="order in finished" :key="order.id">
                    <div class="p-3">
                      <n-space justify="space-between">
                        <n-text type="info">{{ order.user }}</n-text>
                        <n-text type="info">{{ order.time }}</n-text>
