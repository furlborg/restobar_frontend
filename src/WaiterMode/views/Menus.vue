    ...
    <n-spin :show="loading">
      <n-list v-if="scheduledMenus.length > 0" class="m-0">
        <n-list-item
          ...
          :key="menu.id"
          class="menu-item"
          :class="{ 'no-stock': getAvailableItemsCount(menu.items) === 0 }"
          ...
          <n-thing>
            <template #header>
              <n-space align="center" justify="space-between">
                <n-space align="center">
                  ...
                  <n-badge 
                    v-if="getAvailableItemsCount(menu.items) === 0"
                    value="Sin stock"
                    type="error"
                  />
                  <n-badge 
                    v-else-if="getAvailableItemsCount(menu.items) < menu.items?.length"
            ...
            <template #description>
              <n-space vertical size="small">
                ...
                <n-space size="small">
                  <n-tag
                    v-for="(products, phase) in getGroupedProducts(menu.items)"
                    :key="phase"
                    size="small"
                    type="info"
                  >
