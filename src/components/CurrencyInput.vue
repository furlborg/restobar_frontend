  ...
  <div class="currency">
    <label class="currency-wrapper" :class="{ shake: shouldShake }">
      ...
      <div class="input-section">
        <input class="fs-1" ref="divisor" v-model="amount_value" @keydown="onKeyDown" @input="applyMask($event)"
          :maxlength="maxLength" v-bind="vAutowidth" />
        <input ref="remainder" v-show="remainder > 0 || showRemainder" v-model="remainder" @keydown="onRemainderKeydown"
          maxlength="2" />
...
const vAutowidth = VueInputAutowidth;
...
const onRemainderKeydown = (e) => {
  ...
  if ((isBackspace && !remainder.value) || (isArrowKey(e) && isAtInputStart))
    shiftBack(e);
};
